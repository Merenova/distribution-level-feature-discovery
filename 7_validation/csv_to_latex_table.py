...
METRIC_SHORT_NAMES = {
    ...
    "sum_diff_eps-1.0_prefix_mean": r"$\epsilon_{-1.0}$",
    "sum_diff_eps-0.5_prefix_mean": r"$\epsilon_{-0.5}$",
    "sum_diff_eps-0.1_prefix_mean": r"$\epsilon_{-0.1}$",
    "sum_diff_eps0.1_prefix_mean": r"$\epsilon_{0.1}$",
    "sum_diff_eps0.5_prefix_mean": r"$\epsilon_{0.5}$",
    "sum_diff_eps1.0_prefix_mean": r"$\epsilon_{1.0}$",
}
...
# Default metrics including all eps values
DEFAULT_METRICS = [
    "centered_logit_spearman_mean",
    "centered_logit_corr_mean",
    "sum_diff_eps-1.0_prefix_mean",
    "sum_diff_eps-0.5_prefix_mean",
    "sum_diff_eps-0.1_prefix_mean",
    "sum_diff_eps0.1_prefix_mean",
    "sum_diff_eps0.5_prefix_mean",
    "sum_diff_eps1.0_prefix_mean",
]
...
def generate_transposed_table(
    ...
    # Count correlation metrics for multirow adjustment
    num_corr_metrics = sum(1 for m in metrics if m in CORRELATION_METRICS)
    num_effect_metrics = num_metrics - num_corr_metrics
    ...
    # Data rows: for each method, iterate through metrics
    for method_idx, method in enumerate(methods):
        method_display = METHOD_NAMES.get(method, method)
        
        # Separate correlation and effect metrics
        corr_metrics = [m for m in metrics if m in CORRELATION_METRICS]
        effect_metrics = [m for m in metrics if m not in CORRELATION_METRICS]
        
        # First output correlation metrics (with gray background starting from metric column)
        for corr_idx, metric in enumerate(corr_metrics):
            row_values = []
            ...
            if corr_idx == 0:
                method_cell = f"\\multirow{{{num_metrics + 1}}}{{*}}{{\\textbf{{{method_display}}}}}"
            else:
                method_cell = ""
        ...
        # Add cmidrule to separate correlation from effect metrics
        lines.append(f"\\cmidrule(lr){{2-{total_data_cols + 2}}}")
        
        # Then output effect metrics (no background)
        for effect_idx, metric in enumerate(effect_metrics):
            row_values = []
            ...
            # Build row string (no cellcolor for effect metrics)
            metric_name = METRIC_SHORT_NAMES.get(metric, metric[:8])
            row_str = f" & {metric_name} & " + " & ".join(row_values) + r" \\"
            lines.append(row_str)
    ...
    lines.append(r"\caption{Steering results by $\beta$ and $\gamma$ configuration. " +
                ...
                r"Gray rows: correlation metrics; white rows: steering effect at strength $\epsilon$. " +
                r"\textbf{RD}: RD medoid steering; \textbf{KM-Sem}: semantic-only K-means; " +
                r"\textbf{Single}: single continuation direction.}")
...
def generate_by_sweep_table(df, metrics, precision, output_path) -> str:
    ...
    if metrics is None:
        metrics = DEFAULT_METRICS[:5]  # Use first 5 metrics for sweep
