...
Intervention = tuple[
    ...
    int | torch.Tensor,
    int | float | torch.Tensor | FeatureSteeringSpec,
]


def _infer_input_n_positions(
    model,
    inputs,
    original_activations: torch.Tensor | None,
    using_past_kv_cache_idx: int | None,
) -> int:
    if using_past_kv_cache_idx is not None and using_past_kv_cache_idx > 0:
        return 1
    if original_activations is not None:
        return original_activations.size(1)
    if isinstance(inputs, torch.Tensor):
        if inputs.ndim == 1:
            return int(inputs.size(0))
        if inputs.ndim == 2 and inputs.size(0) == 1:
            return int(inputs.size(1))
        raise ValueError(f"Expected a single sequence tensor, got shape {tuple(inputs.shape)}")
    if isinstance(inputs, list) and all(isinstance(token, int) for token in inputs):
        return len(inputs)
    return len(model.tokenizer(inputs).input_ids)


def _compute_feature_intervention_delta(current_value, value):
    if not isinstance(value, FeatureSteeringSpec):
        return value - current_value

    h_c_norm = value.h_c_norm if value.h_c_norm else 1.0
    if value.steering_method == "additive":
        return value.epsilon * (value.h_c_value / h_c_norm)
    if value.steering_method == "multiplicative":
        return current_value * value.epsilon * (value.h_c_value / h_c_norm)
    if value.steering_method == "absolute":
        return value.epsilon - current_value
    if value.steering_method == "sign":
        h_c_sign = 1.0 if value.h_c_value > 0 else -1.0 if value.h_c_value < 0 else 0.0
        return h_c_sign * current_value * value.epsilon
    if value.steering_method == "scaling":
        return value.epsilon * current_value
    raise ValueError(f"Unknown steering method: {value.steering_method}")
...
class NNSightReplacementModel(LanguageModel):
    ...
    def _perform_feature_intervention(
        ...
        n_pos = _infer_input_n_positions(
            self,
            inputs,
            original_activations,
            using_past_kv_cache_idx,
        )
        ...
        for layer in range(self.cfg.n_layers):
            if interventions_by_layer[layer]:
                ...
                for pos, feature_idx, value in interventions_by_layer[layer]:
                    current_value = transcoder_activations[pos, feature_idx]
                    activation_deltas[pos, feature_idx] = _compute_feature_intervention_delta(
                        current_value,
                        value,
                    )
